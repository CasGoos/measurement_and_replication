...
@flake2022construct descriptively investigated QMPs within the reproducibility project psychology [@osc2015estimating]. This study seeks to conceptually replicate some of their findings within the Many Labs replication projects.
...
The replication datasets refer to the data of each of the replication attempts for each of the original psychological effects undertaken as part of a Many Labs study. The specific data was extracted from the replication datasets for each relevant measure. The measure had to consist of multiple scalar items to be considered relevant for the related analyses. Single item measures were not usable for the analyses on score reliability [@wanous1996estimating] as they do not translate directly to the findings of @stanley2014expectations, which this study is following up on.
...
# for hypothesis 2
data_h2 <- data_prep_H2(data_1.10_clean, data_1.11_clean, data_1.12.3.1_clean, 
...
extracted_data_list <- list(data_1.10_clean, data_1.11_clean, data_1.12.3.1_clean,
                            data_1.12.3.2_clean, data_2.10.1_clean,
                            data_2.12.1_clean, data_2.12.2_clean, 
                            data_2.12.3_clean, data_2.15_clean, data_2.20_clean,
                            data_2.23_clean, data_3.2.1.1_clean, 
                            data_3.2.1.2_clean, data_3.7.1_clean, 
                            data_3.7.2_clean, data_3.8.2_clean,
                            data_5.1.1_clean, data_5.7_clean, data_5.9.1_clean)
...
The mean sample size within the replications was `r apa_num(mean(coded_data_replications$N, na.rm = TRUE), digits = 0)` participants (IQR = `r apa_num(IQR(coded_data_replications$N, na.rm = TRUE), digits = 0)`). In the original articles the mean sample size was `r apa_num(mean(coded_data_original_shortened$N, na.rm = TRUE), digits = 0)` (IQR = `r apa_num(IQR(coded_data_original_shortened$N, na.rm = TRUE), digits = 0)`). In total, support was found for hypothesized effects of `r apa_num(table(coded_data_replications$hypothesis_support)["Yes"], digits = 0)` of the `r apa_num(nrow(coded_data_replications), digits = 0)` measures. Proportions of replication success per Many Labs project can be found in Table \@ref(tab:ReplicationRatioTable).
...
ggplot(plot_23_data_alpha, aes(x = alpha, y = g)) +
  ...
  annotation_custom(grob = textGrob(label = "Not Replicated", hjust = 0),
    ymin = 7, ymax = 7, xmin = 1.08, xmax = 1.08) +
...
```{r Plot23OmegaCode, include = FALSE, eval = FALSE}
# plot for omega
ggplot(plot_23_data_omega, aes(x = alpha, y = g)) +
  geom_boxplot(outlier.shape = NA) +
  geom_hline(yintercept = 6.5, color = "red", size = 1) +
  geom_point(alpha = 0.1) +
  theme_minimal() +
...
The counts of the QMP ratio obtained for each QMP type (see Table \@ref(tab:QMPCodingInfoTable)) are shown across the top layer of Figure \@ref(fig:Plot456Code) for both replication protocols and original articles. The overall number of counts for the QMP ratios of the QMP modification type were lower compared to the others, because only `r apa_num(table(coded_data_replications$mod_check)["True"])` out of `r apa_num(length(coded_data_replications$mod_check))` measures in replications, and only `r apa_num(table(coded_data_original_shortened$mod_check)["True"], numerals = FALSE)` measures used in original studies were modified. The bottom layer of Figure \@ref(fig:Plot456Code) shows the overlayed distributions of the total QMP ratios from the replication protocols and original studies, with dots indicating each datapoint, and the line indicating the mean for either distribution.

```{r Plot456Code, warning = FALSE, fig.cap = "QMP ratio counts for each QMP Type and QMP total ratio distribution grouped by where each was coded from. The top row shows binned counts of QMP ratio’s that were observed for each replication protocol and original article, across the different QMP types. The bottom graph shows the distributions of total QMP ratios for both replication protocols and original articles, with the line indicating the mean QMP ratio. The specific observed values are indicated along the bottom row with dots"}
# plot for alpha
### ratio (REV) Original and Rep
Plot_456_1 <- ggplot(plot_456_data[plot_456_data$QMP_type != "Total",], 
       aes(x = QMP_ratio, fill = RepOrg)) + 
  geom_bar(position = "dodge", stat = "bin", bins = 10) +
  facet_grid(~QMP_type) +
  theme_minimal() +
...
A similar beta-regression tested the association between replication success and QMP ratio in replication protocols. Results showed that successful replication significantly related to a decrease in the ratio of QMPs in replication protocols (`r betareg_output_to_apa_full(H5_test_results_REV)`). Sensitivity analyses were performed using a random-intercept multilevel regression function. This model also found a significant association between replication success and QMP ratio (`r check_h5_REV$full_result$hypothesis_supportTRUE`). Both of these results are in line with hypothesis 5. However, the relationship in the multilevel model is no longer significant for QMP ratios obtained with the initial coding protocol (`r check_h5$full_result$hypothesis_supportTRUE`).

The relation between QMP ratios in the replication protocols and corresponding original articles were investigated using a beta regression model. It showed that total QMP ratio in the original article can significantly predict total QMP ratio in the subsequent replication (`r betareg_output_to_apa_full(H6_test_results_REV)`), which provides some evidence in favor of hypothesis 6. However, this relation was not significant for the QMP ratios obtained with the initial protocol (`r betareg_output_to_apa_full(H6_test_results)`). Sensitivity analyses using a random intercept logistic multilevel model also found some discrepancy in results between revised and initial protocol, albeit with both being non-significant (revised protocol: `r check_h6_REV$full_result$QMP_REV`; initial protocol `r check_h6$full_result$QMP`).

```{r Plot46InitialDataCode, include = FALSE, eval = FALSE, error = FALSE}
# Initial
...
Figure \@ref(fig:Plot46RevisedDataCode) visually illustrates the relations of QMP ratios from different QMP types between replication protocols and original articles further.
...
cfa_results_expanded <- cbind(c("Caruso et al. (2012)", "Husnu & Crisp (2010)", "Nosek et al. (2002), Art", "Nosek et al. (2002), Math", "Van Lange et al. (1997)", "Anderson et al. (2012), SWL", "Anderson et al. (2012), PA", "Anderson et al. (2012), NA", "Giessner & Schubert (2007)", "Norenzayan et al. (2002)", "Zhong & Liljenquist (2006)", "Monin & Miller (2001), most", "Monin & Miller (2001), some", "Cacioppo et al. (1983), elm", "Cacioppo et al. (1983), nfc", "De Fruyt et al. (2000), consc", "Albarracín et al. (2008), exp. 5", "Shnabel & Nadler (2008)", "Vohs & Schooler (2008)"), cfa_results)
...
cfa_results_for_print$RMSEA[16] <- "Did"
cfa_results_for_print$`N Factors`[16] <- "Not"
cfa_results_for_print$Unidimensional[16] <- "Converge"
