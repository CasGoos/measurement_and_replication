...
Suitable data for factor analyses was available for `r apa_num(nrow(cfa_results), numerals = FALSE)` measures. These measures were the same as those shown in Figure \@ref(fig:Plot23AlphaCode). The results are shown in Table \@ref(tab:UnidimesnionalityTestResultsTable). A composite score based on the measurement responses was used for sixteen of the `r apa_num(nrow(cfa_results), numerals = FALSE)` measures to form a single variable in the analyses, and were thus regarded as intended to be unidimensional. The number of dimensions for the remaining three were unclear. Sufficient fit of the unidimensional model was found for `r apa_num(sum(cfa_results$Single_factor == "Yes", na.rm = TRUE), numerals = FALSE)` out of the `r apa_num(nrow(cfa_results), numerals = FALSE)` evaluated measures. However, as shown in Table \@ref(tab:UnidimesnionalityTestResultsTable) the unidimensional model held based on the parallel analysis test alone, for only `r apa_num(sum(cfa_results$N_factors == 1, na.rm = TRUE), numerals = FALSE)` of the `r apa_num(nrow(cfa_results), numerals = FALSE)` measures. For those `r apa_num(sum(cfa_results$N_factors == 1, na.rm = TRUE), numerals = FALSE)` measures and an additional `r apa_num(sum(cfa_results$RMSEA < .08, na.rm = TRUE) - sum(cfa_results$N_factors == 1, na.rm = TRUE), numerals = FALSE)` the RMSEA $< .08$ criteria suggested evidence for single factor structures.
