...
for (i in 1:length(extracted_data_list)){
  ...
  single_factor <- ifelse(RMSEA_value < 0.08 | parallel_N_factors == 1, TRUE, 
                          FALSE)
...
```{r Plot 46 Initial Data Code, include = FALSE, eval = FALSE}
# Initial
...
```{r}
#apa_num(sum(table(coded_data_original_shortened$N_items, coded_data_original_shortened$reliability_type)["multiple item measure", "alpha"]), numerals = FALSE)

# $\left(B = -0.963, t(70.319) = -1.491, p = .011 \right)$
