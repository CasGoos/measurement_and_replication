...
```{r Cleaning Replication Datasets Data, include = FALSE, eval = FALSE}
...
```{r Cleaning Coded Data, include = FALSE, eval = FALSE}
##### Data already available, code does not need to be run!
...
apa_table(
  ...
  , escape = FALSE, placement= "ht", booktabs = TRUE)
...
```{r Check for Reliability and Validity Testability, include = FALSE}
...
```{r Final Preparation of Data, include = FALSE, eval = FALSE}
##### Data already available, code does not need to be run!
...
```{r Hypothesis 1 Model, include = FALSE}
...
\end{align}
```
```{r Hypothesis 2 Model, include = FALSE}
...
```{r ICC calculations, include = FALSE}
# alpha
...
```{r Hypothesis 3 Main Test, include = FALSE}
...
```{r Hypothesis 3 Multilevel Model Sensitivity Analyses, include = FALSE}
...
The reported analyses were primarily based on the revised set of QMP data. The results using the original QMP data are mentioned when the results led to different conclusions. The analysis code for both the original and revised QMP data are available in the R script on both the OSF page (<https://osf.io/9r8yt/>) and github page (<https://github.com/CasGoos/measurement_and_replication.git>) of this article.
...
```{r Hypothesis 4 Model, include = FALSE}
...
```{r Hypothesis 5 Model, include = FALSE}
## Hypothesis 4 beta regression model
...
```{r Hypothesis 6 Model, include = FALSE}
...
```{r Hypotheses 5 & 6 Sensitivity Analyses, include = FALSE}
...
```{r Hypothesis 6 Associations, include = FALSE}
...
```{r Unidimensionality Exploratory Analysis, include = FALSE}
...
apa_table(
  ...
  , note = "$^a$replication was assessed as unclear for three of the measures, since their effect was not fully replicated. These have been treated as not replicated within this table and further analyses throughout the thesis"
  , escape = FALSE, placement = "ht", midrules = 4)
...
```{r Plot23AlphaCode, fig.cap = "Distributions of calculated Cronbach’s alpha coefficients (> 0) calculated for the responses on a measure at each lab location, across the eighteen distinct measures for which suitable raw data was available to calculate Cronbach’s alpha coefficients from."}
# plot for alpha
...
```{r ReplicabilityReliabilityTestTable}
...
apa_table(
  ...
  , escape = FALSE, placement= "ht", midrules = c(1, 2, 5, 7, 8, 9, 12, 15))
...
```{r Plot46RevisedDataCode, error = FALSE, fig.cap = "Scatterplot of original and replication QMP ratio’s per type with linear regression line. The abbreviations on the axes relate to the QMP categories described in Table 1: Def. is definition, Op. is operationalization, Sel. is selection/creation, Quant. is quantification, and Mod. is modification. The X axis facets across the QMP ratios in the original articles, and the Y axis facets across the QMP ratios in the replication protocols. Each dot in the figure relates to describes the QMP ratio for the types in that graph across an original article and its replication protocol. Note: jitter was applied to these dots in order to show the number of observations at points where multiple dots were present"}
# Revised
...
```{r H6ExploratoryAssociationsRevisedTable}
...
```{r UnidimesnionalityTestResultsTable}
...
for Measure Data", note = "N Factors refer to the number of factors suggested based on parallel analysis of the measure data. The word or abbreviation after the author names and year indicates the specific measure within that original article that was analysed.", escape = FALSE, row.names = FALSE, placement= "ht")
