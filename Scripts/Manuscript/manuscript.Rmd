...
H2_test_result_alpha <- summary(lmer(formula = alpha ~ 1 + (1|g), data = data_h2))

# and same analyses for omega
# intercept only model to check the within and between group variance.
H2_test_result_omega <- summary(lmer(formula = omega.tot ~ 1 + (1|g), data = data_h2[complete.cases(data_h2),]))
...
# alpha
# within group variance: 0.04384
# between group variance: 0.19824
ICC_alpha <- 0.19824 / (0.04384 + 0.19824)

# omega
# within group variance: 0.003779
# between group variance: 0.034025
ICC_omega <- 0.034025 / (0.003779 + 0.034025)
...
H3_test_result_main_alpha <- summary(glm(formula = replication ~ 1 + alpha_avg, 
                                     family = binomial(), data = data_h3_avg))

# and for omega
H3_test_result_main_omega <- summary(glm(formula = replication ~ 1 + 
                                     omega.tot_avg, family = binomial(), 
                                     data = data_h3_avg))
...
H3_test_result_int_sensitivity_alpha <- summary(glmer(formula = replication ~ 1 
                                                + alpha + (1|g), family = 
                                                binomial(), data = 
                                                data_h3_multiple))

# and for omega
H3_test_result_int_sensitivity_omega <- summary(glmer(formula = replication ~ 1 
...
H3_test_result_slp_sensitivity_alpha <- summary(glmer(formula = replication ~ 1 
                                                + alpha + (1 + alpha|g), 
                                                family = binomial(), data =
                                                data_h3_multiple))

# and for omega
H3_test_result_slp_sensitivity_omega <- summary(glmer(formula = replication ~ 1 
