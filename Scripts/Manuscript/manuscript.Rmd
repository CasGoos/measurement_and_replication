...
![Figure 1. Reliability reporting flow diagram](../../SupplementaryMaterials/reliability_reporting_flow_diagram.png)
...
```{r Plot 23 Omega Code, include = FALSE, eval = FALSE}
# plot for omega
...
```{r Replicability and Reliability Test Table}
...
Cronbach's alpha and omega did not significantly relate to replication success in the main logistic regression model (Cronbachs alpha: `r H3_test_result_main_alpha$full_result$alpha`; omega: `r H3_test_result_main_omega$full_result$omega`), nor in any of the multilevel sensitivity analyses models (see Table XXX). This is contradictory to the expectation in hypothesis 3. However, the results from these analyses should be evaluated with caution, given the small sample number of measures used to estimate these models.
...
```{r Plot 46 Initial Data Code, include = FALSE, eval = FALSE, error = FALSE}
# Initial
...
```{r Plot 46 Revised Data Code, error = FALSE}
# Revised
...
```{r H6 Exploratory Associations Revised Table}
...
```{r H6 Exploratory Associations Initial Table}
# creating the dataframe for the table
...
```{r Unidimesnionality Test Results Table}
