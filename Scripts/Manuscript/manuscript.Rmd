...
apa_table(
  ...
  , note = "$^a$ refers only to the number of identified primary measures, which this study is concerned with. Rep. = Replicaiton, Org. = Original., N = number of, IQR = Interquartile range.")
```
...
The idea here is that when information on or support for the use of a measurement is lacking, any subsequent replication attempt is expected to also lack information on the measure. As a result, both the underreporting of measurement details and use of unverified measurements may persist in the replication. Consequently, ensuring that the original measurement is mimicked becomes difficult to verify when the measurement was not documented well originally. In turn, the relation between the original and replication studies may weaken [@flake2022construct], and as a result, constructs are measured substantially differently, thus the subsequent test in the replication is substantially different from the test in the original article. This would mean that the observed effects in the replication may differ from the original not only due to sampling error, but also due to true differences in the assessed effect as the result of the measurement used.
...
As a final note on QMPs, it is worth highlighting a specific aspect about the way the QMP ratios were calculated. This relates to the fact that non-applicable items, while adjusted for by coding those items as 0 in both the numerator and denominator of the QMP ratio calculation, could exert a strong influence on the value of the QMP ratio. For example, for one measure all relevant information regarding the measure was reported, for another no information was provided on the measure at all and the items were all non-applicable. In both cases the QMP ratio would be 0, however we would not consider the latter case to be nearly as informative as the first. As a result the relation between QMP ratio and measurement reporting quality is obscured. There is no simple solution to this problem based on teh current data, however future research may wish to include qualitative analyses of the informativeness of measurement detail reporting to provide a more thorough and complete picture.

The small sample of measures for which reliability coefficients could be assessed limited the degree to which substantial interpretations can be made from the reliability analyses. While future research will also unlikely be able to increase the number of multiple item measures used by replications, future research can broaden their analyses to include a larger number of replications, or take into account a type of measure which this study mostly ignored in the analyses of reliability and validity: single-item measures.
...
Finally, this paper investigated the consistency of measures, by assessing the variance in reliability across different labs wherein the measure was conducted, and comparing it to the variance in reliability between the different measures. Besides comparing the degree of reliability across labs, it is also possible to assess whether a measure assesses the same underlying construct across lab locations. This can be done through measurement invariance testing. Measurement invariance is a concept in latent factor modelling that illustrates the extent to which the relationship between items and factors is dependent on some other variable, usually a group [@mellenbergh1989item]. @shaw2020measurement investigated measurement invariance for the responses to measures at each lab location within the replication sets of Many Labs 2. Investigations similar to this have also been suggested for future replication projects [@flake2022construct]. For these reasons, future research should consider using measurement invariance tests, when assessing measurement consistency in replications across different labs.

## Implications for Research and Replications in Psychological Science

Taking all the findings together, the following assessments will be relevant to consider for future replication research. With respect to the quality of measurement reporting, the small number of reported reliabilities, and lack of reported validity evidence corroborate findings by @shaw2020measurement & @flake2022construct. Evidence on the reliability and validity of the measures used in both replication and original research should be reported more frequently, so that measures can be evaluated and the validity of replication research can be improved.

The results suggest that the use of structured measurement reporting protocols may be an effective way of reducing QMPs, and may explain why the replication protocols had on average lower QMPs than original articles in this study. However, the reliability and validity evidence reporting rate was lower in the Many Labs replication protocols that made use of structured reporting protocols compared to original articles. These protocols could therefore still increase the attention they give to reporting reliability and validity evidence.
