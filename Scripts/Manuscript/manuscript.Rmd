...
```{r Hypothesis 3 Multilevel Model Sensitivity Analyses}
...
```{r Hypotheses 5 & 6 Sensitivity Analyses}
...
```{r Unidimensionality Exploratory Analysis}
...
```{r Code for Descriptives Retrieval}
N_replications <- mean(coded_data_replications$N, na.rm = TRUE)
IQR_replications <- IQR(coded_data_replications$N, na.rm = TRUE)
N_original <- mean(coded_data_original_shortened$N, na.rm = TRUE)
IQR_original <- IQR(coded_data_original_shortened$N, na.rm = TRUE)

N_h_support <- table(coded_data_replications$hypothesis_support)["Yes"]
N_measures <- nrow(coded_data_replications)
```


The mean sample size within the replications was `r apa_num(N_replications, digits = 0)` participants (IQR = `r apa_num(IQR_replications, digits = 0)`). In the original articles the mean sample size was `r apa_num(N_original, digits = 0)` (IQR = `r apa_num(IQR_original, digits = 0)`). In total, support was found for hypothesized effects of `r apa_num(N_h_support, digits = 0)` of the `r apa_num(N_measures, digits = 0)` measures. Proportions of replication success per Many Labs project can be found in Table XXX.
