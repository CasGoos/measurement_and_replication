...
## Descriptives
The mean sample size within the replications was `r printnum(round(mean(coded_data_replications$N, na.rm = TRUE)))` participants (IQR = `r printnum(round(IQR(coded_data_replications$N, na.rm = TRUE)))`). In the original articles the mean sample size was `r printnum(round(mean(coded_data_original_shortened$N, na.rm = TRUE)))` (IQR = `r printnum(round(IQR(coded_data_original_shortened$N, na.rm = TRUE)))`). In total, support was found for hypothesized effects of `r printnum(table(coded_data_replications$hypothesis_support)["Yes"])` of the `r printnum(nrow(coded_data_replications)` measures. Proportions of replication success per Many Labs project can be found in Table XXX.
