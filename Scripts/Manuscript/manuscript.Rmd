...
check_h5 <- lmer(formula = QMP ~ 1 + hypothesis_support + (1|many_labs_version), data = data_h5_sens)
H5_sensitivity_test_results <- summary(check_h5)$coefficients

check_h5_REV <- lmer(formula = QMP_REV ~ 1 + hypothesis_support + (1|many_labs_version), data = data_h5_sens)
H5_sensitivity_test_results_REV <- summary(check_h5_REV)$coefficients
...
check_h6 <- lmer(formula = Rep_QMP ~ 1 + QMP + (1|many_labs_version), data = data_h6_sens)
H6_sensitivity_test_results <- summary(check_h6)$coefficients
  
check_h6_REV <- lmer(formula = Rep_QMP_REV ~ 1 + QMP_REV + (1|many_labs_version), data = data_h6_sens)
H6_sensitivity_test_results_REV <- summary(check_h6_REV)$coefficients

# n of mod ratio's pressent check
...
apa_table(
  ...
  , note = "The Cronbach's alpha section shows the models where the calculated Cronbach's alpha coefficient was the independent variable. In the Omega section it was the calculated omega coefficient. The random slope model for Cronbach's alpha did not converge, thus it results are not shown here. $^* p < .05$"
