...
*Explanation:* contains the results obtained from the exploratory unidimensionality check analyses based on factor analyses of the measures related to the data_1.10_clean, data_1.11_clean, data_1.12.3.1_clean, data_1.12.3.2_clean,data_2.12.1_clean, data_2.12.2_clean, data_2.12.3_clean, data_2.15_clean, data_2.20_clean, data_2.23_clean, data_3.2.1.1_clean, data_3.2.1.2_clean, data_3.7.1_clean, data_3.7.2_clean, data_3.8.2_clean, data_5.1.1_clean, data_5.1.2_clean, data_5.7_clean, & data_5.9.1_clean datasets. This is part of the pre-registered exploratory analyses documented in appendix A.

*Variables:*

-   **RMSEA:** the rmsea value for a single factor confirmatory factor analysis model when in turn applied to each of the datasets under consideration.

-   **N factors:** the number of factors selected for each of the datasets based on the parallel test for selecting factors.

-   **Single_factor:** a boolean variable indicating whether or not the measure was unidimensional based on the criteria that either the rmsea for the single factor model was smaller than .08, or the parallel test returned a single factor solution.
...
-   **omega:** McDonald's omega calculated for a measure at a particular lab location within a single study/questionnaire as extracted from the Many Labs datasets.

-   **ASE:** Standard error of Cronbach's alpha at a particular lab location, based on formulas 2 & 3 from @duhachek2004AlphaStandardError.
...
-   **reporting_index:** index of the row in the data_h456.csv dataset that the indexed measure belongs to.
...
*Explanation*: For each measure, the mean of the variables from data_H23.csv, and the related g variable. Additionally, some data comparing the reported and replicated alpha was added.

*Variables*:

-   **coefficient_reported**: If present, the reported alpha reliability coefficient in the original article for the measure in this row.
...
-   **siginificance_reported_coefficient**: boolean value indicating whether or not the coeffcient_reported value of a measure is outside the 95% percentile of calculated alpha coefficient score of the same measure.

### data_H456_original.csv

*Explanation*: Contain all of the data from the coded_data_original_shortened.csv and one more variable indicating whether reliability was reported or not.

*Variables*:

-   **reliability_reported**: boolean variable indicating whether any response other than nothing or "Not Reported" was given to the **reliability_type** variable.

### data_H456_replication.csv

*Explanation*: Contain all of the data from the coded_data_replications.csv and one more variable indicating whether reliability was reported or not.

*Variables*:

-   **reliability_reported**: boolean variable indicating whether any response other than nothing or "Not Reported" was given to the **reliability_type** variable.

### item_number_change_data.csv

*Explanation:* Contains the many_labs_version, rep_org, title, and mod_5_rev variablefrom the coded_data_replication.csv dataset of the observations where the mod_5_rev variable was TRUE (meaning that the measure's number of items and/or number of response options was modified from original to replication. Additionally, two variables were added to determine whether for that measure the number of items were decreased from original to replication.

*Variables*:

-   **context:** A description of the change in item numbers from original to replication. NA means that for that measure the number of items was not changed.

-   **direction:** A factor indicating whether the number of items was decreased, increased, or no change (NA) from original to replication.

### plot_23_data_alpha.csv

*Explanation*: Contain all of the data from the data_h23.csv that had a calculated alpha level larger than 0. Additionally, it contains a column of alpha scores averaged across many lab locations for each article. Finally, the replication_success was changed to a boolean variable, returning TRUE in case it was coded as "Yes", and as FALSE otherwise.

### plot_23_data_omega.csv

*Explanation*: Contain all of the data from the data_h3_multiple.csv. Additionally, it contains a column of omega scores averaged across many lab locations for each article. Finally, the replication_success was changed to a boolean variable, returning TRUE in case it was coded as "Yes", and as FALSE otherwise.

### plot_456_data.csv

*Explanation*: Contains four variables for each QMP type for all the replication studies and original articles that were coded on QMPs. The rows are ordered to have the original studies first and then the replications, then they are ordered based on QMP type from Definition to Operationalization to Selection to Quantification to Modification to Total. Within each type the QMPs are ordering matches the order of the studies/articles as in the data_h456_replications.csv and data_h456_original.csv.

*Variables:*

-   **QMP_ratio:** float number between 0 and 1 that indicates the QMP ratio based on the initial coding protocol for that QMP type for that study/article.

-   **QMP_ratio_1:** transformed version of the QMP_ratio column, where QMPs for all original articles have a 1 added to their ratio. This was done for the sake of making the figure possible in ggplot as it is right now.

-   **QMP_type:** factor that indicates the QMP type the QMP ratio belongs to (Definition, Operationalization, Selection, Quantification, or Modification).

-   **QMP_ratio_REV:** float number between 0 and 1 that indicates the QMP ratio based on the revised coding protocol for that QMP type for that study/article.

-   **RepOrg:** factor that indicates whether the QMP ratio belongs to a replication or an original article.

-   **QMP_ratio_1_REV:** transformed version of the QMP_ratio_REV column, where QMPs for all original articles have a 1 added to their ratio. This was done for the sake of making the figure possible in ggplot as it is right now.

### plot_46_data.csv

*Explanation*: Contains four variables for each QMP type (including total) for both replication studies and original articles combined per row. The QMP_ratio column indicates the initial QMP ratio for the specific QMP type for that original article. The QMP_type column indicates the specific QMP type the QMP_ratio column belongs to. The QMP_Rep_ratio column indicates the initial QMP ratio for the specific QMP type for that replication study. The QMP_Rep_type column indicates the specific QMP type the QMP_Rep_ratio column belongs to. The rows are ordered so that within each QMP type it matches the order of the studies/articles as in the data_h456_replications.csv and data_h456_original.csv.
