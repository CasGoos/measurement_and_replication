...
-   **g:** index showing that the alpha values belonged to a particular single measure.

-   **reporting_index:** same as above, only now the numbers correspond to the row in the coded_data_replication.csv dataset that the indexed measure belongs to.

### data_H3_avg.csv

*Explanation*: For each measure, the mean of the variables from data_H2.csv, excluding the g indexing variable. Additionally, a boolean indicator that indicated whether or not the original effect was replicated in the many labs replications when looking at the significance of the meta-analytic p-value was also included.
...
-   **coefficient_reported:** the value for the reported alpha coefficient for the specific measure as extracted from the original article the measure was reported in.

-   **coefficient_calculated:** the mean value of the calculated alpha coefficients for the specific measure in the row.

-   **coefficient_difference:** The difference between coefficient_reported and coefficient_calculated.

-   **significance:** boolean value indicating whether or not the coeffcient_reported value of a measure is outside the 95% percentile of calculated alpha coefficient score of the same measure.

### plot_23_data_omega.csv

*Explanation*: Contain all of the data from the data_h3_multiple.csv. Additionally, it contains a column of omega scores averaged across many lab locations for each article. The g indexing variable was changed to reflect the names of the articles were adjusted to look better in the figure this dataset was used for.

### plot_456_data.csv

*Explanation*: Contains four variables for each QMP type for all the replication studies and original articles that were coded on QMPs. The rows are ordered to have the original studies first and then the replications, then they are ordered based on QMP type from Definition to Operationalization to Selection to Quantification to Modification to Total. Within each type the QMPs are ordering matches the order of the studies/articles as in the coded_data_replications.csv and coded_data_original_shortened.csv.
